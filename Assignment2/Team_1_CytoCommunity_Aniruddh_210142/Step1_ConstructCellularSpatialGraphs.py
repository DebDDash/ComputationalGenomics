...
parser.add_argument("-k", "--knn", type=int,
                    required=False, default=80, help="KNN_K")
...
for graph_index in range(0, len(region_name_list)):
    ...
    x_y_coordinates = np.loadtxt(
        GraphCoord_filename, dtype='float', delimiter="\t")

    K = KNN_K
    # should NOT include itself as a nearest neighbor. Checked. "-1" means using all available cores.
    KNNgraph_sparse = kneighbors_graph(
        x_y_coordinates, K, mode='connectivity', include_self=False, n_jobs=-1)
    KNNgraph_AdjMat = KNNgraph_sparse.toarray()
    # Make the graph to undirected.
    # 2min and cost one hundred memory.
    ...
    # save as integers. Checked the bidirectional edges.
...
# generate a vector of unique cell types and store it to .txt for final illustration.
cell_type_vec_uniq = list(set(cell_type_vec))
...
# generate the max number of cells and store this value to .txt for the next step.
max_nodes = math.ceil(max(num_nodes))
...
for graph_index in range(0, len(region_name_list)):
    ...
    node_attr_matrix = np.zeros(
        (len(cell_type_label), len(cell_type_vec_uniq)))
    for cell_ind in range(0, len(cell_type_label)):
        # get the index of the current cell.
        type_index = cell_type_vec_uniq.index(
            cell_type_label["cell_type"][cell_ind])
        # make the one-hot vector for each cell.
        node_attr_matrix[cell_ind, type_index] = 1

    filename1 = ThisStep_OutputFolderName + region_name + "_NodeAttr.txt"
    # save as integers.
    np.savetxt(filename1, node_attr_matrix, delimiter='\t', fmt='%i')
...
for i in range(0, len(region_name_list)):
    ...
    edge_ndarray = np.loadtxt(
        EdgeIndex_filename, dtype='int64', delimiter="\t")
    ...
    # should be float32 not float or float64.
    x_ndarray = np.loadtxt(NodeAttr_filename, dtype='float32', delimiter="\t")
...
dataset = SpatialOmicsImageDataset(
    ThisStep_OutputFolderName, transform=T.ToDense(max_nodes))
print("Step1 done!")
print(datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S'))
